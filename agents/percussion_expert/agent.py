...
async def generate_audio_tool(prompt: str, instrument: str, tool_context: ToolContext):
    ...
    while attempt <= max_retries:
        print(f"Percussion Expert requesting Lyria 2 synthesis (Attempt {attempt + 1})...")
        try:
            ...
            if response.status_code == 200:
                ...
                print("--- LYRIA 2 RAW RESPONSE START ---")
                # print(json.dumps(result, indent=2))
                print("--- LYRIA 2 RAW RESPONSE END ---")
                ...
                if audio_b64:
                    ...
                    # Return special error code to agent for self-healing
                    return "ERROR: PROMPT_FILTERED: The prompt was blocked by safety filters. Please rewrite it to be more compliant (e.g., avoid intense violence or copyrighted name associations) and try again."
...
agent = Agent(
    ...
    name='Percussion_Expert',
    instruction='''
    You specialize in drum patterns, percussion, and rhythm.
    ...
    3. Analyze the error and your original prompt. Identify words that might be too aggressive (e.g., "war", "blood", "killing") or specific artist names.
    4. Rewrite the prompt using more neutral but musically descriptive terms (e.g., instead of "War Drums", use "Epic Industrial Percussion").
    5. Call generate_audio_tool again with the optimized prompt.
    6. Ensure the process is smooth and report only the final result to the Orchestrator.
    ''',
